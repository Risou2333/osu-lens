...
# Validate the upstream response before handing anything to Rust/WASM.
...
# Pass bytes to wasm-bindgen for predictable decoding.
text = text.replace(
...
# Guard suspicious maps before expensive calculations.
...
# Avoid reusing DifficultyAttributes wrappers for Performance calculations.
text = text.replace(
...
# v4 already defaults to the fast hitresult generator.
...
# Track wrappers that actually need manual free(). BeatmapAttributesBuilder.build()
# consumes/invalidates the builder's wasm pointer, so calling attrBuilder.free()
# afterwards causes "null pointer passed to rust". Do not free the builder manually.
text = text.replace(
    '    let difficulty, perf, diffAttrs, perfAttrs, mapAttrs, attrBuilder, pfcPerfAttrs;',
    '    let difficulty, perf, diffAttrs, perfAttrs, mapAttrs, pfcPerfAttrs;'
)
text = text.replace(
...
# Remove the invalid builder free inserted by an earlier patch.
text = text.replace('        if (attrBuilder) attrBuilder.free();\n', '')
...
if text == original:
    ...
    print('pp-calculator.js patched for rosu-pp-js v4.0.1 (no builder double-free).')
