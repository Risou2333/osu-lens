    ...
    const createSlider = (id, label, min, max, step, value, isHidden = false) => {
        ...
        const performStep = (direction) => {
            ...
            if (direction === 'up' && ['n100', 'n50', 'miss'].includes(id)) {
                ...
                if (n300Input && (parseInt(n300Input.value, 10) || 0) <= 0) {
                    return; // 如果 300s 的数量为 0，则阻止增加 100/50/miss
                }
            }
            ...
            let newValue = direction === 'up'
                ? Math.min(parseFloat(input.max), currentValue + stepValue)
                : Math.max(parseFloat(input.min), currentValue - stepValue);
...
export function setupPpCalculatorListeners() {
    ...
    controls.lazerCheckbox.addEventListener('change', async () => {
        ...
        if (isLazer) {
            // 当启用 Lazer 模式时，自动将 Ticks 和 Ends 设置为最大值
            // 我们需要最新的难度属性来获取这些最大值
